...
const fetchParticipatedProjects = async () => {
  ...
  try {
    const response = await fetch('/api/project/showUserDonateProject', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
...
const fetchInitiatedProjects = async (userId: string) => {
  ...
  try {
    const response = await fetch('/api/project/showUserProject', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
...
const updateProfilePic = async (profilePicBase64: string) => {
  ...
  if (token) {
      try {
          const response = await updateData('/api/profile/updateProfilePic', { profilePic: profilePicBase64 }, token);
          ...
          <Grid item xs={12} style={gridStyle} >
            ...
            {participatedProjects.length > 0 ? (
              participatedProjects.map((project, index) => (
                <ProjectBoxCardMini key={project._id} projectItem={project} index={index} hasCloseBtn={false}/>
              ))
            ) : (
              <EmptyProjectCard />
            )}
